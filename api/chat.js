...
// Lazy pool - only created when needed, avoids crash at module load time
var _pool = null;
function getPool(){
if(!_pool){
var pg = require('pg');
...
try{
var intent=await extractFlightIntent(message,context,apiKey);
if(intent&&intent.is_flight_search&&intent.from_city&&intent.to_city){
var fromAP=await resolveAirport(intent.from_city);
var toAP=await resolveAirport(intent.to_city);
if(fromAP&&toAP){
...
return res.json({action:'show_flights',flights:flights,from:fromAP.city,to:toAP.city,reply:'Found '+flights.length+' flights from '+fromAP.city+' to '+toAP.city+' on '+dateDisp+'.'});
}
...
res.json({reply:reply||'I am not sure about that. Could you rephrase?'});
}catch(err){
